...
bot.on("text", async (msg) => {
  ...
  if (userMessage === "/start") {
    bot.sendMessage(chatId, "به ربات خوش آمدی 👋", {
  ...
  if (userMessage === "🔎 جستجوی نماد") {
    ...
    bot.sendMessage(chatId, "لطفاً نماد مورد نظر را وارد کن (مثلاً BTCIRT)");
    return;
  }
  ...
  if (userMessage === "📊 سبد سرمایه") {
    ...
    if (!userPortfolio || userPortfolio.length === 0) {
      bot.sendMessage(chatId, "سبد شما خالی است. از گزینه «➕ افزودن دارایی» استفاده کن.");
      return;
    }
    ...
    for (const item of userPortfolio) {
      ...
      message += `${status}: ${diff.toFixed(2)} تومان (${percent}%)\n\n`;
    }
  ...
  if (waitingForSymbol[chatId]) {
    ...
    if (price) {
      ...
      bot.sendMessage(chatId, "قیمت این نماد پیدا نشد.");
    }
  ...
  if (waitingForAdd[chatId]) {
    ...
    if (step === 1) {
      ...
      bot.sendMessage(chatId, "مرحله ۳: قیمت خرید هر واحد را وارد کن (تومان)");
    } 
    else if (step === 3) {
